...
const Chapters: React.FC = () => {
  ...
  const debouncedUpdateChapter = useCallback(
    async (chapterId: string, newChapterNumber: number) => {
      ...
      debounceTimeoutRef.current = setTimeout(async () => {
        ...
        if (update && id) {
          try {
            ...
            await dispatch(
              fetchChapters({ audiobookId: id, page: currentPage })
            ).unwrap();
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const oldIndex = localChapters.findIndex(
    ...
    const isAtBottom =
      newIndex === localChapters.length - 1 &&
      pagination &&
    ...
    if (isAtTop) {
      ...
      try {
        ...
        const prevPageResult = await dispatch(
          fetchChapters({ audiobookId: id, page: targetPage })
        ).unwrap();
        const sortedPrev = [...(prevPageResult?.response?.data || [])].sort(
          (a, b) => a.chapterNumber - b.chapterNumber
        );
        const lastChapter =
          sortedPrev.length > 0 ? sortedPrev[sortedPrev.length - 1] : null;
        const newChapterNumber = lastChapter
    ...
    if (isAtBottom) {
      ...
      try {
        ...
        const nextPageResult = await dispatch(
          fetchChapters({ audiobookId: id, page: targetPage })
        ).unwrap();
        const sortedNext = [...(nextPageResult?.response?.data || [])].sort(
          (a, b) => a.chapterNumber - b.chapterNumber
        );
        const newChapterNumber =
          sortedNext.length > 0 && sortedNext[0]
            ? Math.max(1, sortedNext[0].chapterNumber - 1)
            : 1;
    ...
    if (newIndex === 0) {
      ...
      newChapterNumber = nextChapter
        ? Math.max(1, nextChapter.chapterNumber - 1)
        : 1;
      ...
      if (prevChapter && nextChapter) {
        newChapterNumber = Math.floor(
          (prevChapter.chapterNumber + nextChapter.chapterNumber) / 2
        );
